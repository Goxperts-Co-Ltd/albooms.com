...
//		$res = @iconv('utf-8', 'cp1252//TRANSLIT//IGNORE', $textLow);
	...
	public static function render(StoreNavigation $request, $options) {
		$reqCat = StoreModule::resolveCategoryByRequest($request);
		$reqItem = StoreModule::resolveItemByRequest($request, !!$reqCat);
		
		$cat = $options->category ? StoreData::getCategory($options->category) : $reqCat;
		$request->category = $cat ? $cat : null;
		$request->categoryKey = null;
		
		$item = ($request->item && $options->category && !StoreData::categoryHasItem($options->category, $request->item->id)) ? null : $reqItem;
