...
class StoreModule {
	...
	public static function getNextInvoiceDocumentNumber($format) {
		...
		$seriesId = mb_strtoupper(preg_replace("#{(R?N)(?::\\d+)?}#isu", "{\\1}", $seriesId), "utf-8");
		$resettableCounterKey = mb_strtoupper(preg_replace("#{(R?N)(?::\\d+)?}#isu", "{\\1}", $resettableCounterKey), "utf-8");
	 ...
	 */
	public static function resolveItemByRequest(StoreNavigation $request, $hasCategory = null) {
		if (is_null($hasCategory)) {
			$hasCategory = !!self::resolveCategoryByRequest($request);
		}
		if ($hasCategory) {
			$itemKey = (isset($request->args[1]) && $request->args[1]) ? $request->args[1] : null;
		} else {
			$itemKey = (isset($request->args[0]) && $request->args[0]) ? $request->args[0] : null;
		}
	...
	private static function handleStoreNav($reqInf) {
		...
		if (in_array('store', $pageCtrls)) {
			...
			self::$storeNav->item = self::resolveItemByRequest(self::$storeNav, !!self::$storeNav->category);
			if (self::$storeNav->item) {
				...
				if( !empty(self::$storeNav->item->image) ) {
					if( is_string(self::$storeNav->item->image) )
						...
						$reqInf->image = self::$storeNav->item->image->image;
					else if( !empty(self::$storeNav->item->image->thumb) )
						$reqInf->image = self::$storeNav->item->image->thumb;
				}
				$forcedHome = false;
			}
